const Game = (io, games, utils, cardsPerPlayer, scoreToWin) => {
  const Game = {
    ...
    toggleReady: ({ room, userName }) => {
      ...
      games[gameIndex].players[playerIndex].ready =
        !games[gameIndex].players[playerIndex].ready;

      const readyPlayers = games[gameIndex].players.filter(
        (player) => player.ready == true
      );

      if (
        readyPlayers.length == games[gameIndex].players.length &&
        games[gameIndex].canStart == false &&
        games[gameIndex].players.length > 1
      ) {
    ...
    dealPhraseCards: (roomNum) => {
      ...
      players.forEach((player) => {
        if (!player.interviewer) {
          const cardPack = games[gameIndex].phraseCards.slice(
            0,
            cardsPerPlayer
          );
