...
const Game = {
  updatePlayerList: (roomNum, games) => {
    const game = games[utils.getGameIndex(roomNum, games)];
  ...
  toggleReady: (roomNum, socket, games) => {
    const playersGame = games[utils.getGameIndex(roomNum, games)];
    ...
    const readyPlayers = playersGame.players.filter(
      (player) => player.ready == true
    );

    if (
      readyPlayers.length == playersGame.players.length &&
      playersGame.canStart == false &&
      playersGame.players.length > 1
    ) {
      playersGame.canStart = true;
  ...
  chooseNextInterviewee: (roomNum) => {
    ...
    if (availablePlayers.length > 0) {
      ...
      io.to(roomNum).emit(
        "employmentPhase",
        games[utils.getGameIndex(roomNum)].players
      );
      return -1;
    }
  },
  ...
  nextInterviewer: (roomNum) => {
    ...
    io.to(game.players[game.interviewerIndex].socketId).emit(
      "toggleInterviewer"
    );
